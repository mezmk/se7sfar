...
async def _safe_event_edit(event, *args, **kwargs):
    """edit آمن - لو فشل يبعت رسالة جديدة"""
    kwargs.pop('file', None)
    try:
        return await event.edit(*args, **kwargs)
    except Exception:
        try:
            # الرسالة فيها صورة ومش قادر يعدل → ابعت جديدة
            return await bot.send_message(event.chat_id, *args, **{k:v for k,v in kwargs.items() if k != 'file'})
        except Exception as e:

...
# ============================================================
#  GHOST ANIME PATCH - waifu.pics + waifu.im
# ============================================================
WAIFU_PICS_ENDPOINTS = ["waifu", "neko", "shinobu", "megumin", "cuddle", "hug", "pat", "kiss"]
WAIFU_PICS_GIF = ["cuddle", "hug", "pat", "kiss", "poke", "slap", "blush", "dance", "cringe"]
...
async def _get_anime_media() -> str:
    try:
        ...
        if random.random() < 0.5:
            # صورة من waifu.pics
            ep = random.choice(WAIFU_PICS_ENDPOINTS)
            url = f"https://api.waifu.pics/sfw/{ep}"
            async with session.get(url, timeout=aiohttp.ClientTimeout(total=8)) as r:
                data = await r.json(content_type=None)
                return data["url"]
        else:
            # GIF من waifu.pics
            ep = random.choice(WAIFU_PICS_GIF)
            url = f"https://api.waifu.pics/sfw/{ep}"
            async with session.get(url, timeout=aiohttp.ClientTimeout(total=8)) as r:
                data = await r.json(content_type=None)
                return data["url"]
        ...
        try:
            # fallback: waifu.im
            session = await get_http_session()
            async with session.get("https://api.waifu.im/search/?is_nsfw=false", timeout=aiohttp.ClientTimeout(total=8)) as r:
                data = await r.json(content_type=None)

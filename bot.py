...
# ============================================================
#  GHOST ANIME PATCH - catbox static URLs
# ============================================================
ANIME_IMAGES = [
    "https://files.catbox.moe/tg6m59.jpg",
    "https://files.catbox.moe/p7a3l0.jpg",
    "https://files.catbox.moe/wyh3cb.jpg",
    "https://files.catbox.moe/5d45io.jpg",
    "https://files.catbox.moe/qcd0nm.jpg",
    "https://files.catbox.moe/cexrnv.jpg",
    "https://files.catbox.moe/e3t3xt.jpg",
    "https://files.catbox.moe/cw2eq1.jpg",
    "https://files.catbox.moe/bisf5n.jpg",
    "https://files.catbox.moe/wi1ri6.jpg",
    "https://files.catbox.moe/2i6mhv.jpg",
    "https://files.catbox.moe/7e4axz.jpg",
    "https://files.catbox.moe/0n840t.jpg",
    "https://files.catbox.moe/qu1n4b.jpg",
    "https://files.catbox.moe/0em7c5.jpg",
    "https://files.catbox.moe/pg02i1.jpg",
    "https://files.catbox.moe/mnssxf.jpg",
    "https://files.catbox.moe/t35hhe.jpg",
    "https://files.catbox.moe/wi8wvv.jpg",
    "https://files.catbox.moe/v3yupj.jpg",
    "https://files.catbox.moe/3xu8hd.jpg",
    "https://files.catbox.moe/y70227.jpg",
    "https://files.catbox.moe/ffcq6c.jpg",
    "https://files.catbox.moe/t2qvep.jpg",
    "https://files.catbox.moe/2gx73e.jpg",
    "https://files.catbox.moe/ifwxbq.jpg",
    "https://files.catbox.moe/kov7qa.jpg",
    "https://files.catbox.moe/ot3u8j.jpg",
    "https://files.catbox.moe/869aam.jpg",
    "https://files.catbox.moe/cht2o8.jpg",
    "https://files.catbox.moe/z05q6s.jpg",
    "https://files.catbox.moe/o7wwy9.jpg",
    "https://files.catbox.moe/on1j2b.jpg",
]
...
async def _anime_reply_injector(event):
    ...
    async def patched_reply(*args, **kwargs):
        msg_text = args[0] if args else kwargs.get('message', '')
        if isinstance(msg_text, str) and len(msg_text) > 900:
...
async def start_mass_check(user_id, cards, sites, event):
    ...
    if user_id not in ADMIN_ID and is_banned(user_id):
        await _safe_event_edit(event, premium_emoji("❌ You are banned from this bot."), parse_mode='html')
        return
    ...
    if not sites:
        await _safe_event_edit(event, premium_emoji("❌ Nᴏ sɪᴛᴇs ᴀᴠᴀɪʟᴀʙʟᴇ!"), parse_mode='html')
        return
    proxies = get_user_proxies(user_id)
    if not proxies:
        await _safe_event_edit(event, premium_emoji("❌  Nᴏ ᴘʀᴏxɪᴇs!\n\nᴜsᴇ /ᴀᴅᴅᴍʏᴘʀᴏxʏ ᴛᴏ ᴀᴅᴅ ʏᴏᴜʀ ᴘʀᴏxɪᴇs"), parse_mode='html')
        return
    status_msg = await _safe_event_edit(event, premium_emoji(f"🔥 Sᴛᴀʀᴛɪɴɢ ᴄʜᴇᴄᴋ ғᴏʀ {len(cards)} ᴄᴀʀᴅs..."), parse_mode='html')
...
async def start(event):
    user_id = event.sender_id
    if user_id not in ADMIN_ID and is_banned(user_id):
        await event.reply(premium_emoji("❌ You are banned from this bot.")
, parse_mode='html')
        return
    is_prem = is_premium(user_id)
...
└─ <code>/redeem Kᴇʏ</code> → Rᴇᴅᴇᴇᴍ ᴀ ᴘʀᴇᴍɪᴜᴍ ᴋᴇʏ """
    ...
    await _safe_event_edit(event, premium_emoji(commands_text), buttons=buttons, parse_mode='html')
...
└─ <code>/hits</code> → Tᴏɢɢʟᴇ ʜɪᴛs ᴏɴ/ᴏғғ"""
    buttons = [[Button.inline(" Bᴀᴄᴋ", b"main_menu", style="danger", icon=5445365692004071819)]]
    await _safe_event_edit(event, premium_emoji(admin_text), buttons=buttons, parse_mode='html')
...
━━━━━━━━━━━━━━━━━"""
    ...
    buttons = get_main_menu_keyboard(user_id)
    await _safe_event_edit(event, premium_emoji(welcome_text), buttons=buttons, parse_mode='html')
...
└─ <code>/clean</code> → Cʟᴇᴀɴ ᴄᴀʀᴅs (ʀᴇᴍᴏᴠᴇ ᴇxᴘɪʀᴇᴅ)
    ...
    await _safe_event_edit(event, premium_emoji(tools_text), buttons=buttons, parse_mode='html')
...
async def price_filter_callback(event):
    ...
    if user_id not in TEMP_FILE_DATA:
        await _safe_event_edit(event, premium_emoji("❌ Fɪʟᴇ ɴᴏᴛ ғᴏᴜɴᴅ! Pʟᴇᴀsᴇ ᴜᴘʟᴏᴀᴅ ᴀɢᴀɪɴ."), parse_mode='html')
        return
    filters = await load_price_filters()
    gateway_filters = filters.get('shopify_global', DEFAULT_FILTERS)
    ...
    sites_data = await load_sites_with_price()
    if not sites_data:
        await _safe_event_edit(event, premium_emoji("❌ Nᴏ sɪᴛᴇs ғᴏᴜɴᴅ ᴡɪᴛʜ ᴘʀɪᴄᴇs! Rᴜɴ /sɪᴛᴇ ғɪʀsᴛ."), parse_mode='html')
        return
    ...
    if not sites_to_use:
        await _safe_event_edit(event, premium_emoji(f"❌ Nᴏ sɪᴛᴇs ғᴏᴜɴᴅ ɪɴ ʀᴀɴɢᴇ {selected_filter['name']}!"), parse_mode='html')
        return
    await _safe_event_edit(event, premium_emoji(f"🚀 Sᴛᴀʀᴛɪɴɢ ᴄʜᴇᴄᴋ ᴡɪᴛʜ ғɪʟᴛᴇʀ: {selected_filter['name']}\n\n📊 Sɪᴛᴇs: {len(sites_to_use)}\n💳 Cᴀʀᴅs: {len(cards)}"), parse_mode='html')
    ...
    if user_id in TEMP_FILE_DATA:
        ...
        if os.path.exists(file_data['file_path']):
            try:
                ...
                pass
    await _safe_event_edit(event, premium_emoji("❌ Cᴀɴᴄᴇʟʟᴇᴅ."), parse_mode='html')
...
async def split_size_callback(event):
    ...
    await _safe_event_edit(event, premium_emoji(f"🔄 Sᴘʟɪᴛᴛɪɴɢ {len(cards)} ᴄᴀʀᴅs ɪɴᴛᴏ {chunk_size} ᴄᴀʀᴅs ᴘᴇʀ ғɪʟᴇ..."), parse_mode='html')
...
async def split_custom_callback(event):
    ...
    await _safe_event_edit(event, premium_emoji("📝 Sᴇɴᴅ ᴛʜᴇ ɴᴜᴍʙᴇʀ ᴏғ ᴄᴀʀᴅs ᴘᴇʀ ғɪʟᴇ (10-15000):"), parse_mode='html')
...
async def stop_admin_handler(event):
    ...
    if stopped:
        ...
        try:
            await _safe_event_edit(event, premium_emoji(f"✅ Task stopped for user <code>{target_id}</code>"), parse_mode='html')
        except: pass
    else:
...
async def stop_handler(event):
    ...
    if session_key in active_sessions:
        ...
        try:
            await _safe_event_edit(event, premium_emoji("🛑 Cʜᴇᴄᴋɪɴɢ sᴛᴏᴘᴘᴇᴅ ʙʏ ᴜsᴇʀ."), parse_mode='html')
        except:
            pass
...
async def add_my_proxy(event):
    try:
        ...
        status_msg = await event.reply(premium_emoji(f"🔄 Testing {len(new_proxies)} proxies..."), parse_mode='html')
...
async def _safe_event_edit(event, *args, **kwargs):
    """بدل event.edit - لو الرسالة فيها صورة + caption طويل → يمسح ويبعت جديد"""
    try:
        msg_text = args[0] if args else kwargs.get('message', kwargs.get('text', ''))
        if isinstance(msg_text, str) and len(msg_text) > 900:
            # امسح الرسالة القديمة وابعت جديدة
            try:
                await event.delete()
            except:
                pass
        ...
        try:
            return await bot.send_message(event.chat_id, *args, **kwargs)
        except:
            pass
...
async def ban_user_handler(event):
    ...
    parts = event.raw_text.split()
    if len(parts) < 2:
        await event.reply(premium_emoji("Usage: /ban USER_ID"), parse_mode='html')
        return
    try:
        target_id = int(parts[1])
    except:
    ...
    try:
        ...
        uname = str(target_id)
    await event.reply(premium_emoji(f"🚫 User @{uname} (<code>{target_id}</code>) has been banned!"), parse_mode='html')
...
async def unban_user_handler(event):
    ...
    parts = event.raw_text.split()
    if len(parts) < 2:
        await event.reply(premium_emoji("Usage: /unban USER_ID"), parse_mode='html')
        return
    try:
        target_id = int(parts[1])
    except:
    ...
    try:
        ...
        uname = str(target_id)
    await event.reply(premium_emoji(f"✅ User @{uname} (<code>{target_id}</code>) has been unbanned!"), parse_mode='html')
...
async def list_banned_handler(event):
    ...
    banned = load_banned_users()
    if not banned:
        await event.reply(premium_emoji("✅ No banned users."), parse_mode='html')
        return
...
async def forward_to_hits(event):
    try:
        ...
        if not event.is_reply:
            await event.reply(premium_emoji("❌ Reply to any message (photo/voice/file/text) with /f to forward it to Hits channel."), parse_mode='html')
            return
        
        replied = await event.get_reply_message()
        if not replied:
            await event.reply(premium_emoji("❌ No message found to forward."), parse_mode='html')
            return
        ...
        await event.reply(premium_emoji(f"✅ Forward request sent to admin!\n⏳ Waiting for approval..."), parse_mode='html')
    ...
    try:
        ...
        try:
            ...
            if forwarded_msg:
                ...
                await _safe_event_edit(event, "✅ Published & Pinned in Hits channel!")
                # Notify user
                try:
                ...
                await _safe_event_edit(event, "❌ Could not find the message to forward.")
        except Exception as e:
            await _safe_event_edit(event, f"❌ Error: {str(e)[:100]}")
    except Exception as e:
...
async def fwd_reject_handler(event):
    try:
        ...
        await _safe_event_edit(event, "❌ Rejected.")
        # Notify user
        try:

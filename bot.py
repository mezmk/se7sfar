...
async def _anime_send(event, *args, **kwargs):
    sent = await event.reply(*args, **kwargs)
    try:
        media = await _get_anime_media()
        if media:
            await bot.send_file(event.chat_id, file=media, reply_to=event.id)
    except:
        pass
    return sent
# ============================================================
...
async def process_file_with_filters(event, user_id):
    if not event.reply_to_msg_id:
        await _anime_send(event, premium_emoji("❌ Pʟᴇᴀsᴇ ʀᴇᴘʟʏ ᴛᴏ ᴀ .ᴛxᴛ ғɪʟᴇ."), parse_mode='html')
        return
    ...
    try:
        ...
        if not cards:
            await _anime_send(event, premium_emoji("❌ Nᴏ ᴠᴀʟɪᴅ ᴄᴀʀᴅs ғᴏᴜɴᴅ ɪɴ ғɪʟᴇ."), parse_mode='html')
            os.remove(file_path)
            return
        ...
        await _anime_send(event, 
            premium_emoji(f"📁 Fɪʟᴇ ʟᴏᴀᴅᴇᴅ: {len(cards)} ᴄᴀʀᴅs ғᴏᴜɴᴅ!\n\n💰 Sᴇʟᴇᴄᴛ ᴀ ᴘʀɪᴄᴇ ғɪʟᴛᴇʀ:"),
            buttons=buttons,
            parse_mode='html'
        )
    except Exception as e:
        await _anime_send(event, premium_emoji(f"❌ Eʀʀᴏʀ: {e}"), parse_mode='html')
...
async def start(event):
    user_id = event.sender_id
    if user_id not in ADMIN_ID and is_banned(user_id):
        await _anime_send(event, premium_emoji("❌ You are banned from this bot.")
, parse_mode='html')
        return
    is_prem = is_premium(user_id)
...
━━━━━━━━━━━━━━━━━"""
    ...
    try:
        await _anime_send(event, premium_emoji(welcome_text), buttons=buttons, parse_mode='html')
...
async def single_cc_check(event):
    ...
    if not is_premium(user_id):
        await _anime_send(event, premium_emoji("❌ Aᴄᴄᴇss Dᴇɴɪᴇᴅ\n\nOɴʟʏ ᴘʀᴇᴍɪᴜᴍ ᴜsᴇʀs ᴄᴀɴ ᴜsᴇ ᴛʜɪs ʙᴏᴛ."), parse_mode='html')
        return
    # Check stars - need at least 1 star to use bot
    stars = get_stars(user_id)
    if stars <= 0:
        await _anime_send(event, premium_emoji(f"❌ Nᴏᴛ ᴇɴᴏᴜɢʜ sᴛᴀʀs!\n\n⭐ Yᴏᴜ ʜᴀᴠᴇ: {stars} stars\n📅 <i>Daily bonus: {DAILY_STARS} stars</i>\n\n💡 <i>Wait for daily bonus or contact admin</i>"), parse_mode='html')
        return
    sites = load_sites()
    proxies = get_user_proxies(user_id)
    if not proxies:
        await _anime_send(event, premium_emoji("❌  Nᴏ ᴘʀᴏxɪᴇs!\n\nᴜsᴇ /ᴀᴅᴅᴍʏᴘʀᴏxʏ"), parse_mode='html')
        return
    if not proxies:
        await _anime_send(event, premium_emoji("❌ Nᴏ ᴘʀᴏxɪᴇs ᴀᴠᴀɪʟᴀʙʟᴇ!\n\n⚠️ Pʟᴇᴀsᴇ ᴄᴏɴᴛᴀᴄᴛ ᴀᴅᴍɪɴ ᴛᴏ ᴀᴅᴅ ᴘʀᴏxɪᴇsᴇ."), parse_mode='html')
        return
    cc_input = event.message.text.split(' ', 1)[1].strip()
    cards = extract_cc(cc_input)
    if not cards:
        await _anime_send(event, premium_emoji("❌ Iɴᴠᴀʟɪᴅ CC ғᴏʀᴍᴀᴛ. Usᴇ: <code>/cc ᴄᴀʀᴅ|ᴍᴍ|ʏʏ|ᴄᴠᴠ</code>"), parse_mode='html')
        return
    card = cards[0]
    status_msg = await _anime_send(event, premium_emoji(f"🔄 Cʜᴇᴄᴋɪɴɢ <code>{card}</code>..."), parse_mode='html')
    try:
...
async def stripe_charge_single(event):
    ...
    cards = extract_cc(cc_input)
    if not cards:
        await _anime_send(event, premium_emoji("❌ Usage: /st card|mm|yy|cvv"), parse_mode='html')
        return
    card = cards[0]
    proxies = get_user_proxies(user_id)
    proxy = random.choice(proxies) if proxies else None
    status_msg = await _anime_send(event, premium_emoji(f"👑 Checking Stripe Charge...\n<code>{card}</code>\n💰 Price: 1$"), parse_mode='html')
    try:
...
async def vbv_single(event):
    ...
    cards = extract_cc(cc_input)
    if not cards:
        await _anime_send(event, premium_emoji("❌ Usage: /vbv card|mm|yy|cvv"), parse_mode='html')
        return
    card = cards[0]
    status_msg = await _anime_send(event, premium_emoji(f"🪪 Checking VBV Braintree...\n<code>{card}</code>\n💰 Price: 4$"), parse_mode='html')
    try:
...
async def batch_stripe_charge(event):
    user_id = event.sender_id
    if not is_premium(user_id):
        await _anime_send(event, premium_emoji("❌ Aᴄᴄᴇss Dᴇɴɪᴇᴅ"), parse_mode='html')
        return
    if not event.reply_to_msg_id:
        await _anime_send(event, premium_emoji("❌ Reply to a .txt file with cards\n\n📝 Usage: /ran_st (reply to .txt)"), parse_mode='html')
        return
    reply_msg = await event.get_reply_message()
    if not reply_msg.file or not reply_msg.file.name.endswith('.txt'):
        await _anime_send(event, premium_emoji("❌ Please reply to a .txt file"), parse_mode='html')
        return
    file_path = await reply_msg.download_media()
    async with aiofiles.open(file_path, 'r', encoding='utf-8', errors='ignore') as f:
        content = await f.read()
    ...
    os.remove(file_path)
    if not cards:
        await _anime_send(event, premium_emoji("❌ No valid cards found"), parse_mode='html')
        return
    proxies = get_user_proxies(user_id)
    status_msg = await _anime_send(event, premium_emoji(f"👑 FAST Random Check (Stripe Charge)!\n📊 Total: {len(cards)}\n⚡ Speed: 10 simultaneously\n💰 Price: 1$"), parse_mode='html')
...
async def batch_vbv(event):
    ...
    if not cards:
        await _anime_send(event, premium_emoji("❌ No valid cards found"), parse_mode='html')
        return
    status_msg = await _anime_send(event, premium_emoji(f"🪪 FAST Random Check (VBV Braintree)!\n📊 Total: {len(cards)}\n⚡ Speed: 10 simultaneously\n💰 Price: 4$"), parse_mode='html')
...
async def pin_command(event):
    ...
    if len(args) < 2:
        await _anime_send(event, premium_emoji("❌ Usage: /pin <card_number>"), parse_mode='html')
        return
    card_number = args[1].strip().split('|')[0]
    pin = await get_pin_for_card(card_number)
    if pin:
        await _anime_send(event, premium_emoji(f"🔑 PIN Found!\n\n💳 Card: <code>{card_number}</code>\n🔐 PIN: <code>{pin}</code>"), parse_mode='html')
    else:
        await _anime_send(event, premium_emoji(f"❌ No PIN found for <code>{card_number}</code>"), parse_mode='html')
...
async def add_proxy_command(event):
    ...
    if user_id not in ADMIN_ID:
        await _anime_send(event, premium_emoji("❌  Admin only!"), parse_mode='html')
        return
    
    try:
        args = event.message.text.split('\n')
        if len(args) < 2:
            await _anime_send(event, premium_emoji("❌ Usᴀɢᴇ: <code>/addproxy</code> ғᴏʟʟᴏᴡᴇᴅ ʙʏ ᴘʀᴏxɪᴇs, ᴏɴᴇ ᴘᴇʀ ʟɪɴᴇ."), parse_mode='html')
            return
        
        proxies_to_add = [line.strip() for line in args[1:] if line.strip()]
        if not proxies_to_add:
            await _anime_send(event, premium_emoji("❌ Nᴏ ᴘʀᴏxɪᴇs ᴘʀᴏᴠɪᴅᴇᴅ."), parse_mode='html')
            return
        
        status_msg = await _anime_send(event, premium_emoji(f"🔄 Cʜᴇᴄᴋɪɴɢ {len(proxies_to_add)} ᴘʀᴏxɪᴇs ʙᴇғᴏʀᴇ ᴀᴅᴅɪɴɢ..."), parse_mode='html')
...
async def proxy_command(event):
    ...
    if not proxies:
        await _anime_send(event, premium_emoji("❌ Pʀᴏxʏ Nᴏᴛ Fᴏᴜɴᴅ!"), parse_mode='html')
        return
    
    status_msg = await _anime_send(event, premium_emoji(f"🔄 Cʜᴇᴄᴋɪɴɢ {len(proxies)} ᴘʀᴏxɪᴇs..."), parse_mode='html')
...
async def check_single_proxy(event):
    ...
    if not proxy:
        await _anime_send(event, premium_emoji("❌ Usᴀɢᴇ: <code>/chkproxy ɪᴘ:ᴘᴏʀᴛ:ᴜsᴇʀ:ᴘᴀss</code>"), parse_mode='html')
        return
    status_msg = await _anime_send(event, premium_emoji(f"🔄 Cʜᴇᴄᴋɪɴɢ ᴘʀᴏxʏ: <code>{proxy}</code>..."), parse_mode='html')
    try:
        result = await test_proxy(proxy)
        if result['status'] == 'alive':
...
async def remove_single_proxy(event):
    ...
    if not proxy_to_remove:
        await _anime_send(event, premium_emoji("❌ Usᴀɢᴇ: <code>/rmproxy ɪᴘ:ᴘᴏʀᴛ:ᴜsᴇʀ:ᴘᴀss</code>"), parse_mode='html')
        return
    current_proxies = load_proxies()
    if proxy_to_remove not in current_proxies:
        await _anime_send(event, premium_emoji(f"❌ Pʀᴏxʏ ɴᴏᴛ ғᴏᴜɴᴅ: <code>{proxy_to_remove}</code>"), parse_mode='html')
        return
    new_proxies = [p for p in current_proxies if p != proxy_to_remove]
    async with aiofiles.open(PROXY_FILE, 'w') as f:
        for proxy in new_proxies:
            await f.write(f"{proxy}\n")
    await _anime_send(event, premium_emoji(f"✅ Pʀᴏxʏ ʀᴇᴍᴏᴠᴇᴅ!\n\n<code>{proxy_to_remove}</code>"), parse_mode='html')
...
async def remove_proxy_by_index(event):
    ...
    if not indices_str:
        await _anime_send(event, premium_emoji("❌ Usᴀɢᴇ: <code>/rmproxyindex 1,2,3</code>"), parse_mode='html')
        return
    try:
        indices = [int(i.strip()) - 1 for i in indices_str.split(',')]
    except ValueError:
        await _anime_send(event, premium_emoji("❌ Iɴᴠᴀʟɪᴅ ɪɴᴅɪᴄᴇs. Usᴇ ɴᴜᴍʙᴇʀs sᴇᴘᴀʀᴀᴛᴇᴅ ʙʏ ᴄᴏᴍᴍᴀs."), parse_mode='html')
        return
    current_proxies = load_proxies()
    if not current_proxies:
        await _anime_send(event, premium_emoji("❌ Nᴏ ᴘʀᴏxɪᴇs ɪɴ ᴘʀᴏxʏ.ᴛxᴛ"), parse_mode='html')
        return
    removed = []
    new_proxies = []
    ...
    if not removed:
        await _anime_send(event, premium_emoji("❌ Nᴏ ᴠᴀʟɪᴅ ɪɴᴅɪᴄᴇs ғᴏᴜɴᴅ."), parse_mode='html')
        return
    async with aiofiles.open(PROXY_FILE, 'w') as f:
        for proxy in new_proxies:
            await f.write(f"{proxy}\n")
    removed_text = "\n".join(removed[:10])
    await _anime_send(event, premium_emoji(f"✅ Rᴇᴍᴏᴠᴇᴅ {len(removed)} ᴘʀᴏxɪᴇs!\n\nRᴇᴍᴏᴠᴇᴅ:\n<code>{removed_text}</code>"), parse_mode='html')
...
async def clear_all_proxies(event):
    ...
    if count == 0:
        await _anime_send(event, premium_emoji("❌ ᴘʀᴏxʏ.ᴛxᴛ ɪs ᴀʟʀᴇᴀᴅʏ ᴇᴍᴘᴛʏ."), parse_mode='html')
        return
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    ...
    try:
        async with aiofiles.open(backup_filename, 'w') as f:
            for proxy in current_proxies:
                await f.write(f"{proxy}\n")
        await _anime_send(event, premium_emoji(f"📦 Bᴀᴄᴋᴜᴘ ᴄʀᴇᴀᴛᴇᴅ!\n\nSᴇɴᴅɪɴɢ ʙᴀᴄᴋᴜᴘ ᴏғ {count} ᴘʀᴏxɪᴇs..."), file=backup_filename, parse_mode='html')
        try:
            os.remove(backup_filename)
        except:
            pass
    except Exception as e:
        await _anime_send(event, premium_emoji(f"❌ Eʀʀᴏʀ ᴄʀᴇᴀᴛɪɴɢ ʙᴀᴄᴋᴜᴘ: {e}"), parse_mode='html')
        return
    async with aiofiles.open(PROXY_FILE, 'w') as f:
        await f.write("")
    await _anime_send(event, premium_emoji(f"✅ Cʟᴇᴀʀᴇᴅ ᴀʟʟ {count} ᴘʀᴏxɪᴇs!\n\nᴘʀᴏxʏ.ᴛxᴛ ɪs ɴᴏᴡ ᴇᴍᴘᴛʏ."), parse_mode='html')
...
async def get_all_proxies(event):
    ...
    if len(current_proxies) <= 50:
        ...
        await _anime_send(event, premium_emoji(f"📋 Aʟʟ Pʀᴏxɪᴇs ({len(current_proxies)}):\n\n{proxy_list}"), parse_mode='html')
    else:
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
...
async def site_command(event):
    ...
    sites = load_sites()
    if not sites:
        await _anime_send(event, premium_emoji("❌ sɪᴛᴇs.ᴛxᴛ ɪs ᴇᴍᴘᴛʏ."), parse_mode='html')
        return
    proxies = load_proxies()
    if not proxies:
        await _anime_send(event, premium_emoji("❌ Nᴏ ᴘʀᴏxɪᴇs ᴀᴠᴀɪʟᴀʙʟᴇ!\n\n⚠️ Pʟᴇᴀsᴇ ᴄᴏɴᴛᴀᴄᴛ ᴀᴅᴍɪɴ ᴛᴏ ᴀᴅᴅ ᴘʀᴏxɪᴇsᴇ."), parse_mode='html')
        return
    status_msg = await _anime_send(event, premium_emoji(f"🔄 Cʜᴇᴄᴋɪɴɢ {len(sites)} sɪᴛᴇs..."), parse_mode='html')
    alive_sites = []
    dead_sites = []
    sites_with_price = []
    batch_size = 10
...
async def remove_site_command(event):
    ...
    try:
        ...
        if not url_to_remove:
            await _anime_send(event, premium_emoji("❌ Usᴀɢᴇ: <code>/rm ʜᴛᴛᴘs://sɪᴛᴇ.ᴄᴏᴍ</code>"), parse_mode='html')
            return
        current_sites = load_sites()
        if url_to_remove not in current_sites:
            await _anime_send(event, premium_emoji(f"❌ Sɪᴛᴇ ɴᴏᴛ ғᴏᴜɴᴅ: <code>{url_to_remove}</code>"), parse_mode='html')
            return
        new_sites = [site for site in current_sites if site != url_to_remove]
        async with aiofiles.open(SITES_FILE, 'w') as f:
            for site in new_sites:
                await f.write(f"{site}\n")
        await _anime_send(event, premium_emoji(f"✅ Sɪᴛᴇ ʀᴇᴍᴏᴠᴇᴅ!\n\n<code>{url_to_remove}</code>"), parse_mode='html')
    except Exception as e:
        await _anime_send(event, premium_emoji(f"❌ Eʀʀᴏʀ: {e}"), parse_mode='html')
...
async def add_sites_command(event):
    ...
    if user_id not in ADMIN_ID:
        return
    if not event.reply_to_msg_id:
        await _anime_send(event, premium_emoji("📝 Pʟᴇᴀsᴇ ʀᴇᴘʟʏ ᴛᴏ ᴀ .ᴛxᴛ ғɪʟᴇ ᴡɪᴛʜ ᴛʜᴇ ᴄᴏᴍᴍᴀɴᴅ:\n<code>/addsites</code>"), parse_mode='html')
        return
    reply_msg = await event.get_reply_message()
    if not reply_msg.file or not reply_msg.file.name.endswith('.txt'):
        await _anime_send(event, premium_emoji("❌ Pʟᴇᴀsᴇ ʀᴇᴘʟʏ ᴛᴏ ᴀ .ᴛxᴛ ғɪʟᴇ."), parse_mode='html')
        return
    status_msg = await _anime_send(event, premium_emoji("🔄 Pʀᴏᴄᴇssɪɴɢ sɪᴛᴇs ғɪʟᴇ..."), parse_mode='html')
    try:
        file_path = await reply_msg.download_media()
        async with aiofiles.open(file_path, 'r', encoding='utf-8', errors='ignore') as f:
            content = await f.read()
...
async def add_stripe_sites(event):
    ...
    if user_id not in ADMIN_ID:
        return
    if not event.reply_to_msg_id:
        await _anime_send(event, premium_emoji("📝 Reply to a .txt file with Stripe sites\n\nUsage: /addstripe (reply to .txt)"), parse_mode='html')
        return
    reply_msg = await event.get_reply_message()
    if not reply_msg.file or not reply_msg.file.name.endswith('.txt'):
        await _anime_send(event, premium_emoji("❌ Please reply to a .txt file"), parse_mode='html')
    ...
    os.remove(file_path)
    if not sites:
        await _anime_send(event, premium_emoji("❌ No valid sites found"), parse_mode='html')
        return
    ...
    async with aiofiles.open(STRIPE_SITES_FILE, 'w') as f:
        for s in existing:
            await f.write(f"{s}\n")
    await _anime_send(event, premium_emoji(f"✅ Added {added} Stripe sites!\n📊 Total: {len(existing)}"), parse_mode='html')
...
async def rm_stripe_site(event):
    ...
    args = event.message.text.split(maxsplit=1)
    if len(args) < 2:
        await _anime_send(event, premium_emoji("❌ Usage: /rmsite <url>"), parse_mode='html')
        return
    site = args[1].strip()
    ...
    if site in sites:
        ...
        async with aiofiles.open(STRIPE_SITES_FILE, 'w') as f:
            for s in sites:
                await f.write(f"{s}\n")
        await _anime_send(event, premium_emoji(f"✅ Removed: {site}"), parse_mode='html')
    else:
        await _anime_send(event, premium_emoji(f"❌ Site not found: {site}"), parse_mode='html')
...
async def list_stripe_sites(event):
    ...
    if not sites and not shopify_sites:
        await _anime_send(event, premium_emoji("📭 No sites added yet."), parse_mode='html')
        return
    text = ""
    if shopify_sites:
    ...
    await _anime_send(event, premium_emoji(text), parse_mode='html')
...
async def add_premium_command(event):
    ...
    try:
        ...
        if len(parts) < 2:
            await _anime_send(event, premium_emoji("📝 Usᴀɢᴇ: <code>/addpremium ᴜsᴇʀ_ɪᴅ [ʜᴏᴜʀs]</code>\n\n💡 If no hours = permanent"), parse_mode='html')
            return
        ...
        if await add_premium_user(target_id, hours=hours, added_by=user_id):
            ...
            await _anime_send(event, premium_emoji(f"✅ Usᴇʀ <code>{target_id}</code> ᴀᴅᴅᴇᴅ ᴛᴏ ᴘʀᴇᴍɪᴜᴍ ({duration})!"), parse_mode='html')
            try:
                await bot.send_message(target_id, premium_emoji(f"🎉 Yᴏᴜ ʜᴀᴠᴇ ʙᴇᴇɴ ɢʀᴀɴᴛᴇᴅ ᴘʀᴇᴍɪᴜᴍ ᴀᴄᴄᴇss ({duration})!"), parse_mode='html')
            except:
                pass
        else:
            await _anime_send(event, premium_emoji(f"⚠️ Usᴇʀ <code>{target_id}</code> ɪs ᴀʟʀᴇᴀᴅʏ ᴘʀᴇᴍɪᴜᴍ."), parse_mode='html')
    except ValueError:
        await _anime_send(event, premium_emoji("❌ Iɴᴠᴀʟɪᴅ ᴜsᴇʀ ID or hours."), parse_mode='html')
    except Exception as e:
        await _anime_send(event, premium_emoji(f"❌ Eʀʀᴏʀ: {e}"), parse_mode='html')
...
async def remove_premium_command(event):
    ...
    try:
        parts = event.raw_text.split()
        if len(parts) != 2:
            await _anime_send(event, premium_emoji("📝 Usᴀɢᴇ: <code>/removepremium ᴜsᴇʀ_ɪᴅ</code>"), parse_mode='html')
            return
        target_id = int(parts[1])
        if target_id in ADMIN_ID:
            await _anime_send(event, premium_emoji("⚠️ Cᴀɴɴᴏᴛ ʀᴇᴍᴏᴠᴇ ᴀᴅᴍɪɴ ғʀᴏᴍ ᴘʀᴇᴍɪᴜᴍ."), parse_mode='html')
            return
        if await remove_premium_user(target_id):
            await _anime_send(event, premium_emoji(f"✅ Usᴇʀ <code>{target_id}</code> ʀᴇᴍᴏᴠᴇᴅ ғʀᴏᴍ ᴘʀᴇᴍɪᴜᴍ."), parse_mode='html')
            try:
                await bot.send_message(target_id, premium_emoji("⚠️ Yᴏᴜʀ ᴘʀᴇᴍɪᴜᴍ ᴀᴄᴄᴇss ʜᴀs ʙᴇᴇɴ ʀᴇᴠᴏᴋᴇᴅ."), parse_mode='html')
            except:
                pass
        else:
            await _anime_send(event, premium_emoji(f"⚠️ Usᴇʀ <code>{target_id}</code> ɪs ɴᴏᴛ ᴘʀᴇᴍɪᴜᴍ."), parse_mode='html')
    except ValueError:
        await _anime_send(event, premium_emoji("❌ Iɴᴠᴀʟɪᴅ ᴜsᴇʀ ID."), parse_mode='html')
    except Exception as e:
        await _anime_send(event, premium_emoji(f"❌ Eʀʀᴏʀ: {e}"), parse_mode='html')
...
async def genkeys_command(event):
    if event.sender_id not in ADMIN_ID:
        await _anime_send(event, premium_emoji("❌ Aᴄᴄᴇss Dᴇɴɪᴇᴅ. Aᴅᴍɪɴ ᴏɴʟʏ."), parse_mode='html')
        return
    try:
        parts = event.raw_text.split()
        if len(parts) != 4:
            await _anime_send(event, premium_emoji("📝 Usᴀɢᴇ: <code>/genkeys ᴀᴍᴏᴜɴᴛ ʜᴏᴜʀs ᴜsᴇʀ_ʟɪᴍɪᴛ</code>"), parse_mode='html')
            return
        amount = int(parts[1])
        hours = int(parts[2])
        ...
        for idx, key in enumerate(generated_keys, 1):
            keys_text += f"""
┣ <code>{key}</code>"""
        await _anime_send(event, premium_emoji(f"""⭐ <b>Kᴇʏs Gᴇɴᴇʀᴀᴛᴇᴅ</b>   (x{amount})   
━━━━━━━━━━━━━━━━━━
    {keys_text}
┗ 📅 Pᴇʀɪᴏᴅ: {days_display}
           ┗ 👥 Usᴇʀs: {user_limit}
...
async def redeem_key(event):
    ...
    try:
        parts = event.raw_text.split()
        if len(parts) != 2:
            await _anime_send(event, premium_emoji("📝 Usᴀɢᴇ: <code>/redeem Kᴇʏ</code>"), parse_mode='html')
            return
        key = parts[1].upper()
        keys_data = await load_keys()
        if key not in keys_data:
            await _anime_send(event, premium_emoji("❌ Iɴᴠᴀʟɪᴅ Kᴇʏ!"), parse_mode='html')
            return
        ...
        if key_data.get('type') == 'time_limit':
            ...
            if current_date > expiry:
                await _anime_send(event, premium_emoji("❌ Tʜɪs ᴋᴇʏ ʜᴀs EXPIRED!"), parse_mode='html')
                return
            if key_data['used_count'] >= key_data['user_limit']:
                await _anime_send(event, premium_emoji(f"❌ Tʜɪs ᴋᴇʏ ʜᴀs ʀᴇᴀᴄʜᴇᴅ ɪᴛs ʟɪᴍɪᴛ"), parse_mode='html')
                return
            user_id_str = str(user_id)
            if user_id_str in key_data['used_by']:
                await _anime_send(event, premium_emoji("❌ Yᴏᴜ ʜᴀᴠᴇ ᴀʟʀᴇᴀᴅʏ ᴜsᴇᴅ ᴛʜɪs ᴋᴇʏ!"), parse_mode='html')
                return
            if is_premium(user_id):
                await _anime_send(event, premium_emoji("❌ Yᴏᴜ ᴀʟʀᴇᴀᴅʏ ʜᴀᴠᴇ ᴘʀᴇᴍɪᴜᴍ ᴀᴄᴄᴇss!"), parse_mode='html')
                return
            ...
            # Notify user
            await _anime_send(event, premium_emoji(f"""🎉 Cᴏɴɢʀᴀᴛᴜʟᴀᴛɪᴏɴs!
...
async def list_premium_command(event):
    ...
    now = datetime.now()
    if not data:
        await _anime_send(event, premium_emoji("📭 Nᴏ ᴘʀᴇᴍɪᴜᴍ ᴜsᴇʀs ғᴏᴜɴᴅ."), parse_mode='html')
        return
    ...
    await _anime_send(event, premium_emoji(f"👑 <b>Pʀᴇᴍɪᴜᴍ Usᴇʀs ({len(data)})</b>\n{premium_list}"), buttons=buttons if buttons else None, parse_mode='html')
...
🤖 <b>Bᴏᴛ Sᴛᴀᴛᴜs:</b> Rᴜɴɴɪɴɢ ✅"""
    await _anime_send(event, premium_emoji(stats_text), parse_mode='html')
...
async def set_hits_channel(event):
    ...
    try:
        parts = event.raw_text.split()
        if len(parts) != 2:
            await _anime_send(event, premium_emoji("📝 Usᴀɢᴇ: <code>/sethits -1001234567890</code>"), parse_mode='html')
            return
        global HITS_CHANNEL_ID
        HITS_CHANNEL_ID = int(parts[1])
        await _anime_send(event, premium_emoji(f"✅ Hɪᴛs ᴄʜᴀɴɴᴇʟ sᴇᴛ ᴛᴏ: <code>{HITS_CHANNEL_ID}</code>"), parse_mode='html')
    except Exception as e:
        await _anime_send(event, premium_emoji(f"❌ Eʀʀᴏʀ: {e}"), parse_mode='html')
...
async def toggle_hits(event):
    ...
    global HITS_CHANNEL_ID
    if HITS_CHANNEL_ID == 0:
        await _anime_send(event, premium_emoji("❌ Hɪᴛs ᴄʜᴀɴɴᴇʟ ɴᴏᴛ sᴇᴛ. Usᴇ /sᴇᴛʜɪᴛs"), parse_mode='html')
        return
    if HITS_CHANNEL_ID < 0:
        HITS_CHANNEL_ID = abs(HITS_CHANNEL_ID)
        await _anime_send(event, premium_emoji("❌ Hɪᴛs ᴄʜᴀɴɴᴇʟ Tᴜʀɴᴇᴅ Oғғ"), parse_mode='html')
    else:
        HITS_CHANNEL_ID = -abs(HITS_CHANNEL_ID)
        await _anime_send(event, premium_emoji("✅ Hɪᴛs ᴄʜᴀɴɴᴇʟ Tᴜʀɴᴇᴅ Oɴ"), parse_mode='html')
...
async def set_filter_command(event):
    ...
    try:
        ...
        if len(parts) < 4:
            await _anime_send(event, premium_emoji("📝 Usᴀɢᴇ: <code>/setfilter ɢᴀᴛᴇᴡᴀʏ ᴍɪɴ-ᴍᴀx \"Fɪʟᴛᴇʀ Nᴀᴍᴇ\"</code>\n\nExᴀᴍᴘʟᴇ:\n<code>/setfilter shopify_global 0-10 💰 Lᴇss ᴛʜᴀɴ $10</code>"), parse_mode='html')
            return
        gateway = parts[1]
        range_str = parts[2]
        name = parts[3].strip()
        if '-' not in range_str:
            await _anime_send(event, premium_emoji("❌ Iɴᴠᴀʟɪᴅ ʀᴀɴɢᴇ! Usᴇ: ᴍɪɴ-ᴍᴀx"), parse_mode='html')
            return
        ...
        await _anime_send(event, premium_emoji(f"✅ Fɪʟᴛᴇʀ ᴀᴅᴅᴇᴅ: {name}\n💰 {min_val:.0f} - {max_val:.0f}"), parse_mode='html')
    except Exception as e:
        await _anime_send(event, premium_emoji(f"❌ Eʀʀᴏʀ: {e}"), parse_mode='html')
...
async def list_filters_command(event):
    ...
    filters = await load_price_filters()
    if not filters:
        await _anime_send(event, premium_emoji("📭 Nᴏ ғɪʟᴛᴇʀs ғᴏᴜɴᴅ."), parse_mode='html')
        return
...
async def remove_filter_command(event):
    ...
    try:
        parts = event.raw_text.split()
        if len(parts) != 3:
            await _anime_send(event, premium_emoji("📝 Usᴀɢᴇ: <code>/removefilter ɢᴀᴛᴇᴡᴀʏ ɴᴜᴍʙᴇʀ</code>\n\nExᴀᴍᴘʟᴇ:\n<code>/removefilter shopify_global 2</code>"), parse_mode='html')
            return
        gateway = parts[1].lower()
        filter_num = int(parts[2]) - 1
        filters = await load_price_filters()
        if gateway not in filters:
            await _anime_send(event, premium_emoji(f"❌ Nᴏ ғɪʟᴛᴇʀs ғᴏʀ {gateway.upper()}!"), parse_mode='html')
            return
        if filter_num < 0 or filter_num >= len(filters[gateway]):
            await _anime_send(event, premium_emoji(f"❌ Iɴᴠᴀʟɪᴅ ғɪʟᴛᴇʀ ɴᴜᴍʙᴇʀ! Usᴇ 1-{len(filters[gateway])}"), parse_mode='html')
            return
        removed = filters[gateway].pop(filter_num)
        await save_price_filters(filters)
        await _anime_send(event, premium_emoji(f"✅ Fɪʟᴛᴇʀ ʀᴇᴍᴏᴠᴇᴅ:\n┣ 📌 {removed['name']}\n┗ 💰 {removed['min']:.0f}-{removed['max']:.0f}"), parse_mode='html')
    except ValueError:
        await _anime_send(event, premium_emoji("❌ Iɴᴠᴀʟɪᴅ ғɪʟᴛᴇʀ ɴᴜᴍʙᴇʀ!"), parse_mode='html')
    except Exception as e:
        await _anime_send(event, premium_emoji(f"❌ Eʀʀᴏʀ: {e}"), parse_mode='html')
...
async def clean_file(event):
    ...
    status_msg = await _anime_send(event, premium_emoji("🔄 Pʀᴏᴄᴇssɪɴɢ ғɪʟᴇ..."), parse_mode='html')
...
async def merge_files(event):
    ...
    if user_id in MERGE_DATA:
        await _anime_send(event, premium_emoji("⚠️ Yᴏᴜ ᴀʟʀᴇᴀᴅʏ ʜᴀᴠᴇ ᴀɴ ᴀᴄᴛɪᴠᴇ ᴍᴇʀɢᴇ!"), parse_mode='html')
        return
...
async def merge_button(event):
    ...
    if user_id not in MERGE_DATA:
        await _anime_send(event, premium_emoji("❌ Nᴏ ᴀᴄᴛɪᴠᴇ ᴍᴇʀɢᴇ!"), parse_mode='html')
        return
    ...
    if not cards:
        await _anime_send(event, premium_emoji("❌ Nᴏ ᴄᴀʀᴅs ᴄᴏʟʟᴇᴄᴛᴇᴅ!"), parse_mode='html')
        return
    ...
    await _anime_send(event, premium_emoji(f"✅ Mᴇʀɢᴇᴅ {len(cards)} ᴄᴀʀᴅs ғʀᴏᴍ {data['files']} ғɪʟᴇs!"), parse_mode='html')
...
async def extend_merge(event):
    ...
    await _anime_send(event, premium_emoji(f"⏱️ +5 ᴍɪɴᴜᴛᴇs ᴀᴅᴅᴇᴅ!\n📊 Rᴇᴍᴀɪɴɪɴɢ: {remaining} ᴍɪɴᴜᴛᴇs"), parse_mode='html')
...
async def cancel_merge(event):
    ...
    await _anime_send(event, premium_emoji("❌ Mᴇʀɢᴇ Cᴀɴᴄᴇʟʟᴇᴅ!"), parse_mode='html')
...
async def collect_cards(event):
    ...
    if user_id in COLLECT_DATA:
        await _anime_send(event, premium_emoji("⚠️ Yᴏᴜ ᴀʟʀᴇᴀᴅʏ ʜᴀᴠᴇ ᴀɴ ᴀᴄᴛɪᴠᴇ ᴄᴏʟʟᴇᴄᴛɪᴏɴ!"), parse_mode='html')
        return
...
async def collect_button(event):
    ...
    if user_id not in COLLECT_DATA:
        await _anime_send(event, premium_emoji("❌ Nᴏ ᴀᴄᴛɪᴠᴇ ᴄᴏʟʟᴇᴄᴛɪᴏɴ!"), parse_mode='html')
        return
    ...
    await _anime_send(event, premium_emoji(f"✅ Cᴏʟʟᴇᴄᴛᴇᴅ {len(cards)} ᴄᴀʀᴅs!"), parse_mode='html')
...
async def cancel_collect(event):
    ...
    await _anime_send(event, premium_emoji("❌ Cᴏʟʟᴇᴄᴛɪᴏɴ Cᴀɴᴄᴇʟʟᴇᴅ!"), parse_mode='html')
...
async def bin_lookup(event):
    ...
    parts = event.raw_text.split()
    if len(parts) != 2:
        await _anime_send(event, premium_emoji("📝 Usᴀɢᴇ: <code>/bin 411111</code>"), parse_mode='html')
        return
    
    bin_number = parts[1].strip()[:6]
    
    if not bin_number.isdigit() or len(bin_number) < 6:
        await _anime_send(event, premium_emoji("❌ Iɴᴠᴀʟɪᴅ BIN! Eɴᴛᴇʀ ᴀᴛ ʟᴇᴀsᴛ 6 ᴅɪɢɪᴛs."), parse_mode='html')
        return
    
    status_msg = await _anime_send(event, premium_emoji(f"🔄 Lᴏᴏᴋɪɴɢ ᴜᴘ BIN <code>{bin_number}</code>..."), parse_mode='html')
...
async def gen_cards(event):
    ...
    parts = event.raw_text.split()
    if len(parts) < 2:
        await _anime_send(event, premium_emoji("""📝 Usᴀɢᴇ: <code>/gen BIN [count]</code>
...
<code>/gen 415920|12|2028|123 10</code>"""), parse_mode='html')
    ...
    try:
        ...
        if not prefix.isdigit() or len(prefix) < 6:
            await _anime_send(event, premium_emoji("❌ Iɴᴠᴀʟɪᴅ BIN! Mᴜsᴛ ʙᴇ ᴀᴛ ʟᴇᴀsᴛ 6 ᴅɪɢɪᴛs."), parse_mode='html')
            return
        
        status_msg = await _anime_send(event, premium_emoji(f"🔄 Gᴇɴᴇʀᴀᴛɪɴɢ {count} ᴄᴀʀᴅs..."), parse_mode='html')
...
async def stripe_key_check(event):
    ...
    parts = event.raw_text.split()
    if len(parts) != 2:
        await _anime_send(event, premium_emoji("""📝 Usᴀɢᴇ: <code>/sk &lt;Stripe_Key&gt;</code>
...
<code>/sk sk_live_xxxxxxxxxxxxxxxxxxxx</code>"""), parse_mode='html')
    ...
    if not key.startswith(('pk_live_', 'pk_test_', 'sk_live_', 'sk_test_')):
        await _anime_send(event, premium_emoji("❌ Iɴᴠᴀʟɪᴅ Sᴛʀɪᴘᴇ Kᴇʏ!\n\nMᴜsᴛ sᴛᴀʀᴛ ᴡɪᴛʜ:\n<code>pk_live_</code>, <code>pk_test_</code>, <code>sk_live_</code>, ᴏʀ <code>sk_test_</code>"), parse_mode='html')
        return
    
    status_msg = await _anime_send(event, premium_emoji(f"🔄 Cʜᴇᴄᴋɪɴɢ Sᴛʀɪᴘᴇ Kᴇʏ..."), parse_mode='html')
...
async def site_check(event):
    ...
    parts = event.raw_text.split()
    if len(parts) != 2:
        await _anime_send(event, premium_emoji("""📝 Usᴀɢᴇ: <code>/scg &lt;URL&gt;</code>
...
<code>/scg example.com</code>"""), parse_mode='html')
    ...
    status_msg = await _anime_send(event, premium_emoji(f"🔍 Sᴄᴀɴɴɪɴɢ <code>{url}</code>..."), parse_mode='html')
...
async def fake_data(event):
    ...
    parts = event.raw_text.split()
    if len(parts) != 2:
        await _anime_send(event, premium_emoji("""📝 Usᴀɢᴇ: <code>/fake &lt;ᴄᴏᴜɴᴛʀʏ_ᴄᴏᴅᴇ&gt;</code>
...
"""), parse_mode='html')
    ...
    status_msg = await _anime_send(event, premium_emoji(f"🔄 Gᴇɴᴇʀᴀᴛɪɴɢ ғᴀᴋᴇ ᴅᴀᴛᴀ ғᴏʀ <code>{country_code}</code>..."), parse_mode='html')
...
async def ip_lookup(event):
    ...
    if not data:
        await _anime_send(event, premium_emoji("""📝 Usᴀɢᴇ: <code>/ip &lt;IP_Address&gt;</code>
...
"""), parse_mode='html')
    ...
    if not ip_match:
        await _anime_send(event, premium_emoji("❌ Iɴᴠᴀʟɪᴅ IP Aᴅᴅʀᴇss!"), parse_mode='html')
        return
    
    ip_address = ip_match.group(0)
    
    status_msg = await _anime_send(event, premium_emoji(f"🔄 Lᴏᴏᴋɪɴɢ ᴜᴘ <code>{ip_address}</code>..."), parse_mode='html')
...
async def iban_lookup(event):
    ...
    if not data:
        await _anime_send(event, premium_emoji("""📝 Usᴀɢᴇ: <code>/iban &lt;IBAN&gt;</code>
...
"""), parse_mode='html')
    ...
    if not iban_match:
        await _anime_send(event, premium_emoji("❌ Iɴᴠᴀʟɪᴅ IBAN!"), parse_mode='html')
        return
    
    iban = iban_match.group(0).replace(' ', '')
    
    status_msg = await _anime_send(event, premium_emoji(f"🔄 Cʜᴇᴄᴋɪɴɢ <code>{iban}</code>..."), parse_mode='html')
...
async def add_my_proxy(event):
    try:
        user_id = event.sender_id
        if not is_premium(user_id):
            await _anime_send(event, premium_emoji("❌  Access Denied\n\nOnly premium users can use this bot."), parse_mode='html')
            return
        ...
        if not new_proxies:
            await _anime_send(event, premium_emoji("❌  Usage:\n/addmyproxy ip:port:user:pass\nOR\n/addmyproxy\nip:port:user:pass"), parse_mode='html')
            return
        current = get_user_proxies(user_id)
        added = [p for p in new_proxies if p not in current]
        if added:
            save_user_proxies(user_id, current + added)
            await _anime_send(event, premium_emoji(f"✅  Added {len(added)} proxies!\nTotal: {len(current)+len(added)}"), parse_mode='html')
        else:
            await _anime_send(event, premium_emoji("⚠️  All proxies already exist!"), parse_mode='html')
    except Exception as e:
        import traceback
        print(f'[ERROR add_my_proxy] {e}')
        traceback.print_exc()
        try:
            await _anime_send(event, premium_emoji(f"❌  Error: {str(e)}"), parse_mode='html')
        except:
            pass
...
async def add_my_proxy(event):
    try:
        ...
        status_msg = await _anime_send(event, premium_emoji(f"🔄 Testing {len(new_proxies)} proxies..."), parse_mode='html')
...
async def add_my_proxy(event):
    ...
    proxies = get_user_proxies(user_id)
    if not proxies:
        await _anime_send(event, premium_emoji("❌  No proxies! Use /addmyproxy"), parse_mode='html')
        return
    await _anime_send(event, premium_emoji(f"✅  Your proxies: {len(proxies)}\n\n" + "\n".join(proxies[:10])), parse_mode='html')

@bot.on(events.NewMessage(pattern=r'/clearproxy'))
async def clear_my_proxy(event):
    user_id = event.sender_id
    save_user_proxies(user_id, [])
    await _anime_send(event, premium_emoji("✅  All your proxies cleared!"), parse_mode='html')
...
async def ban_user_handler(event):
    user_id = event.sender_id
    if user_id not in ADMIN_ID:
        await _anime_send(event, premium_emoji("❌ Admins only!"), parse_mode='html')
        return
    parts = event.raw_text.split()
    if len(parts) < 2:
        await _anime_send(event, premium_emoji("Usage: /ban USER_ID"), parse_mode='html')
        return
    try:
        target_id = int(parts[1])
    except:
        await _anime_send(event, premium_emoji("❌ Invalid user ID!"), parse_mode='html')
        return
    ...
    try:
        ...
        uname = str(target_id)
    await _anime_send(event, premium_emoji(f"🚫 User @{uname} (<code>{target_id}</code>) has been banned!"), parse_mode='html')
...
async def unban_user_handler(event):
    ...
    parts = event.raw_text.split()
    if len(parts) < 2:
        await _anime_send(event, premium_emoji("Usage: /unban USER_ID"), parse_mode='html')
        return
    try:
        target_id = int(parts[1])
    except:
    ...
    try:
        ...
        uname = str(target_id)
    await _anime_send(event, premium_emoji(f"✅ User @{uname} (<code>{target_id}</code>) has been unbanned!"), parse_mode='html')
...
async def list_banned_handler(event):
    ...
    banned = load_banned_users()
    if not banned:
        await _anime_send(event, premium_emoji("✅ No banned users."), parse_mode='html')
        return
...
async def forward_to_hits(event):
    try:
        ...
        if not event.is_reply:
            await _anime_send(event, premium_emoji("❌ Reply to any message (photo/voice/file/text) with /f to forward it to Hits channel."), parse_mode='html')
            return
        
        replied = await event.get_reply_message()
        if not replied:
            await _anime_send(event, premium_emoji("❌ No message found to forward."), parse_mode='html')
            return
        ...
        await _anime_send(event, premium_emoji(f"✅ Forward request sent to admin!\n⏳ Waiting for approval..."), parse_mode='html')

...
@bot.on(events.NewMessage(pattern='/testanime'))
async def test_anime(event):
    await event.reply("🔍 Testing APIs...")
    
    # test nekos.best
    try:
        import aiohttp
        async with aiohttp.ClientSession() as s:
            async with s.get("https://nekos.best/api/v2/neko", timeout=aiohttp.ClientTimeout(total=10)) as r:
                data = await r.json()
                img_url = data["results"][0]["url"]
                await bot.send_file(event.chat_id, file=img_url, caption=f"✅ nekos.best OK\n{img_url}", reply_to=event.id)
    except Exception as e:
        await event.reply(f"❌ nekos.best FAIL: {e}")
    
    # test otakugifs
    try:
        async with aiohttp.ClientSession() as s:
            async with s.get("https://otakugifs.xyz/gif?reaction=fight&format=gif", timeout=aiohttp.ClientTimeout(total=10)) as r:
                data = await r.json()
                gif_url = data["url"]
                await bot.send_file(event.chat_id, file=gif_url, caption=f"✅ otakugifs OK\n{gif_url}", reply_to=event.id)
    except Exception as e:
        await event.reply(f"❌ otakugifs FAIL: {e}")
